...
if($orderby=='') { $orderby=TABLE_SHOW."_ID"; }
if($ascdesc=='') { $ascdesc='DESC'; }
############################################################################
if($myAjaxAction=="data-delete" && $myAjaxID>0) {
	$sql=" DELETE FROM ".TABLE_SHOW." WHERE ".TABLE_SHOW."_ID='".$myAjaxID."' ";
...
if($task=="load-edit" && $myAjaxID>0) {
	// ---------------------------------------
	$sql=" SELECT * FROM ".TABLE_SHOW." WHERE ".TABLE_SHOW."_ID='".$myAjaxID."' LIMIT 0,1 ";
	$Query=MYSQL_QUERY($sql,$System_Connection1) OR DIE("Error: ".$sql."<br>\n");
	$Row = mysql_fetch_array($Query);
	// ---------------------------------------
	$myID=$myAjaxID;
	$myTable=TABLE_SHOW;
	$myKeyField=TABLE_SHOW."_ID";
	?>
	<!-------------------------------------------------------------------------------->
	<div class="form-group width-100">
		<!--------------------------->
		<?php $myField=TABLE_SHOW."_Name"; ?>
		...
		<!--------------------------->
		<?php $myField=TABLE_SHOW."_Detail"; ?>
		<label class="col-xs-12 col-sm-4 col-md-4 col-lg-4 control-label">
		...
		<!--------------------------->
		<?php $myField=TABLE_SHOW."_Image_Url"; ?>
		<label class="col-xs-12 col-sm-4 col-md-4 col-lg-4 control-label">
		...
		<!--------------------------->
		<?php $myField=TABLE_SHOW."_StartTime"; ?>
		<label class="col-xs-12 col-sm-4 col-md-4 col-lg-4 control-label">
		...
		<!--------------------------->
		<?php $myField=TABLE_SHOW."_EndTime"; ?>
		<label class="col-xs-12 col-sm-4 col-md-4 col-lg-4 control-label">
...
if($myAjaxAction=="data-update") {
	$sql = " UPDATE ".TABLE_SHOW." SET ".$myAjaxKey."='".$myAjaxValue."' WHERE ".TABLE_SHOW."_ID='".$myAjaxID."' ";
	$Query=MYSQL_QUERY($sql,$System_Connection1) OR DIE("Error: ".$sql."<br>\n");
	echo " <script> ";
...
if($task=="load-add") {
	// Check uncomplete ------------------
	$sql=" SELECT ".TABLE_SHOW."_ID FROM ".TABLE_SHOW." WHERE ".TABLE_SHOW."_Image_Url='' LIMIT 0,1 ";
	$Query=MYSQL_QUERY($sql,$System_Connection1) OR DIE("Error: ".$sql."<br>\n");
	$Row = mysql_fetch_array($Query);
	$myUnCompleteID=$Row[0];
	// ---------------------------------------
	if($myUnCompleteID>0) { } else{ // insert new
		$sql=" SELECT MAX(".TABLE_SHOW."_ID) FROM ".TABLE_SHOW." WHERE 1 ";
		$Query=MYSQL_QUERY($sql,$System_Connection1) OR DIE("Error: ".$sql."<br>\n");
		$Row = mysql_fetch_array($Query);
		$myUnCompleteID=$Row[0]+1;
		$sql=" INSERT INTO ".TABLE_SHOW."(".TABLE_SHOW."_ID,".TABLE_SHOW."_Image_Url) VALUES('".$myUnCompleteID."','') ";
		$Query=MYSQL_QUERY($sql,$System_Connection1) OR DIE("Error: ".$sql."<br>\n");
	}
	// ---------------------------------------
	$sql=" SELECT * FROM ".TABLE_SHOW." WHERE ".TABLE_SHOW."_ID='".$myUnCompleteID."' LIMIT 0,1 ";
	$Query=MYSQL_QUERY($sql,$System_Connection1) OR DIE("Error: ".$sql."<br>\n");
	$Row = mysql_fetch_array($Query);
	$myUnCompleteID=$Row[TABLE_SHOW."_ID"];
	// ---------------------------------------
	$myID=$myUnCompleteID;
	$myTable=TABLE_SHOW;
	$myKeyField=TABLE_SHOW."_ID";
...
if($doaction=="result") {
	if($search=='') {
		$sql=" SELECT * FROM ".TABLE_SHOW." WHERE ".TABLE_SHOW."_Name<>'' ";
	}else{
		$sql=" SELECT * FROM ".TABLE_SHOW." WHERE ".TABLE_SHOW."_Name LIKE '%".$search."%' OR ".TABLE_SHOW."_Detail LIKE '%".$search."%'";
	}
	...
	while($Row = mysql_fetch_array($Query)) {
		?>
		<tr
		onclick=" $('#idActionBT<?php echo $Row[TABLE_SHOW."_ID"];?>').show(); "
		onmouseover=" $('#idActionBT<?php echo $Row[TABLE_SHOW."_ID"];?>').show(); "
		onmouseout=" $('#idActionBT<?php echo $Row[TABLE_SHOW."_ID"];?>').hide(); "
		id="idTableRow<?php echo $Row[TABLE_SHOW."_ID"];?>">
		<td><?php echo $Row[TABLE_SHOW."_ID"];?></td>
		<td><?php echo $Row[TABLE_SHOW."_Name"];?></td>
		<td><?php echo $Row[TABLE_SHOW."_Detail"];?></td>
		<td><?php echo $Row[TABLE_SHOW."_Image_Url"];?></td>
		<td><?php echo $Row[TABLE_SHOW."_StartTime"]; ?></td>
		<td><?php echo $Row[TABLE_SHOW."_EndTime"]; ?></td>
		<td style=" width:60px; padding: 0px; ">
		<div id="idActionBT<?php echo $Row[TABLE_SHOW."_ID"];?>"
		style=" height: 30px; padding: 4px; width: 140px; position: absolute; margin-top: -14px; margin-left: -80px; text-align: right; display: none; ">
			...
			onclick=" showPopUpWindowEdit(<?php echo $Row[TABLE_SHOW."_ID"];?>); "><i class="fa fa-pencil-square-o"></i> Edit</button>
			<button type="button" class="btn btn-danger btn-flat"
			style=" font-size:10px; padding:2px; padding-left:10px; padding-right: 10px; "
			onclick=" showPopUpWindowDelete(<?php echo $Row[TABLE_SHOW."_ID"];?>); "><i class="fa fa-times"></i> Delete</button>
		</div>
		</td>
		</tr>
		<?php
...
if($doaction=="count") {
	if($search=='') {
		$sql=" SELECT COUNT(*) FROM ".TABLE_SHOW." WHERE ".TABLE_SHOW."_User<>'' AND ".TABLE_SHOW."_Pass<>'' ";
	}else{
		$sql=" SELECT COUNT(*) FROM ".TABLE_SHOW." WHERE ".TABLE_SHOW."_User LIKE '%".$search."%' OR ".TABLE_SHOW."_Detail LIKE '%".$search."%'";
	}
	$Query=MYSQL_QUERY($sql,$System_Connection1) OR DIE("Error: ".$sql."<br>n");
	$Row = mysql_fetch_array($Query);
	echo $Row[0];
