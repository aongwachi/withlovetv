...
if($SystemSession_Staff_ID>0) { 
	...
	if($Row[TABLE_STAFF."_FileFolder"]<>"") {
	    $_SESSION['SESSION_FILEMAN_USERPATH']="/upload/files/".$Row[TABLE_STAFF."_FileFolder"];
	} else {
	    $_SESSION['SESSION_FILEMAN_USERPATH']="/upload/files/".$Row[TABLE_STAFF."_ID"];
	}
    }
    if(!file_exists("..".$_SESSION['SESSION_FILEMAN_USERPATH'])){ mkdir("..".$_SESSION['SESSION_FILEMAN_USERPATH']); chmod("..".$_SESSION['SESSION_FILEMAN_USERPATH'],0777); }
    //----------------------------------------------------------------------------------------------------------------
    $index=0;
    ...
    //----------------------------------------------------------------------------------------------------------------
    if($act=="draft" && $cid>0) {
	$sql1=" UPDATE ".TABLE_CONTENT." SET ".TABLE_CONTENT."_Status='Draft',".TABLE_CONTENT."_OnlineDate='".SYSTEM_DATETIMENOW."' WHERE ".TABLE_CONTENT."_ID='".$cid."' ";
	...
	$sql1=" UPDATE ".TABLE_CONTENT." SET ".TABLE_CONTENT."_Status='Publish',".TABLE_CONTENT."_OnlineDate='".SYSTEM_DATETIMENOW."' WHERE ".TABLE_CONTENT."_ID='".$cid."' ";
	$Query1=MYSQL_QUERY($sql1,$System_Connection1) OR DIE("Error: ".$sql1."<br>\n");
	?>
	<form name="myRedirectForm" id="myRedirectForm" method="get" action="allcontent.php">
	</form>
			...
			for($i=0;$i<=sizeof($arLayout);$i++) {
				...
				if($myLayoutCheck<>"") {
					...
					################################################################################################
						...
						<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 text-left" style=" padding-left:0px; ">
							<div class="box-sky text-center padding-5"><br><font color=red><b> ห้าม ตั้งชื่อไฟล์และโฟลเดอร์ เป็น ภาษาไทย หรือ เว้นวรรค </b></font></div>
							...
							//----------------------------------------
							    ...
							    tinymce.init({
									...
									language: "th_TH",
								    content_css: [
									    '/templates/bb/css/layout_index19.css',
									    '/templates/bb/css/layout_topmenu2.css',
									    '/templates/bb/css/layout_editor.css'
								    ],
			...
			<div class="form-group width-100" style=" padding-left:0px; ">
				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 text-left" style=" padding-left:0px; ">
					<div class="form-group pull-left" style="">
						<!--------------------------->
						<label class="col-xs-6 col-sm-4 col-md-4 col-lg-4 control-label">
							<span class="label-main">แสดงแบบสุ่ม</span>
						</label>
						<div class="col-xs-12 col-sm-8 col-md-8 col-lg-8 custom-text-position">
							<?php
							$myField=TABLE_CONTENT."_isRandomAds"; 
							$Config_UniqueID=$myField.$myID;
							$Config_DefaultValue=$Row[$myField];
							$Config_DB="UPDATE#".$myTable."#SET#".$myField."#=xxx#WHERE#".$myKeyField."#=#".$myID;
							//--------------------------------------------
