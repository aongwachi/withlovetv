...
$sql="SELECT * FROM ".TABLE_CONTENT." WHERE ".TABLE_CONTENT."_Thumb<>'' AND ".TABLE_CONTENT."_Subject<>'' AND ".TABLE_CONTENT."_Text<>'' ";
$sql.=" AND ".TABLE_CONTENT."_Status='Publish' AND ".TABLE_CONTENT."_Category LIKE '%,26,%' ";
$sql.=" AND ".TABLE_CONTENT."_OnlineDate<='".$myDelayTime."' ";
$sql.=" ORDER BY ".TABLE_CONTENT."_OnlineDate DESC LIMIT 0,100 ";
...
while($Row=$query->fetch()) {
	...
	if(strpos(" ".$myTextAll,"<img ")>0) {
		...
		for($s=1;$s<sizeof($arImg);$s++) {
			...
			//------------------------------
			if($myRefName<>"") {
				$myTextAllImg.='<figcaption class="op-vertical-center">';
				$myTextAllImg.='<h1 class="op-vertical-above op-center">ภาพจาก '.$myRefName.'</h1>';
				$myTextAllImg.='</figcaption>';
	...
	//--------------------------------	
	$myTextAlls=trim(strip_tags($myTextAll));
