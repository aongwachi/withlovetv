...
$myMenuKey="Menu-Setting";
$System_AjaxFileAction="ajax-menu-setting-loaddata.php";
...
if($SystemSession_Staff_ID>0 && $SystemSession_Staff_Level=="Admin") { 
	...
	$FooTableASCDESC='ASC';
	$FooTableOrderBy=TABLE_MENU."_Ordering";
	//----------------------------------------------------------------------------------------------------------------
        $index=1;
        $sql=" SELECT * FROM ".TABLE_MENU." WHERE ".TABLE_MENU."_Name<>'' AND ".TABLE_MENU."_Link<>'' AND ".TABLE_MENU."_Type='".$ModuleConfig_Type."' ORDER BY ".TABLE_MENU."_Ordering ASC ";
	$Query=MYSQL_QUERY($sql,$System_Connection1) OR DIE("Error: ".$sql."<br>\n");
	while($Row = mysql_fetch_array($Query)) {
            $myID=$Row[TABLE_MENU."_ID"];
            $sql1=" UPDATE ".TABLE_MENU." SET ".TABLE_MENU."_Ordering='".$index."' WHERE ".TABLE_MENU."_ID='".$myID."' ";
	...
	<span class="glyphicon glyphicon-plus"></span> เพิ่มเมนูใหม่</button>
	</div>
	...
	<div class="panel-heading panel-custom-heading font-white" style=" background-color:<?php echo $myBGColor; ?>; color:#FFFFFF; text-align:left; height:80px; position:relative; ">
		<i class="glyphicon glyphicon-th-list pull-left padding-10" style="font-size:40px; padding-right:20px;"></i>
		<h2 class="font-white"><strong>Setting Menu</strong></h2> <span> รายชื่อเมนู</span>
	...
	<div class="panel-body panel-custom-body padding-10" style="padding-top:10px; padding-bottom:10px; " id="idTableCalendar"> 
		...
		<div class="input-group width-100 padding-5" style="padding-left:0px; padding-right:0px;">
			<input type="text" id="search" name="search" class="form-control border-info font-18 text-center" style=" height:40px; " 
			 placeholder="ค้นหาจาก ชื่อเมนู หรือ ลิงค์">
		...
		<th data-toggle="true" style="max-width:60px;">
                <a href="javascript:void(0)" class="Link_FooTable" data-sort="<?php echo TABLE_MENU."_ID"; ?>"><i></i>#</a></th>
		<th style=" width:40px; ">Icon</th>
		<th><a href="javascript:void(0)" class="Link_FooTable" data-sort="<?php echo TABLE_MENU."_Name"; ?>"><i></i>Name</a></th>
		<th><a href="javascript:void(0)" class="Link_FooTable" data-sort="<?php echo TABLE_MENU."_Link"; ?>"><i></i>Link</a></th>
		<th><a href="javascript:void(0)" class="Link_FooTable" data-sort="<?php echo TABLE_MENU."_Level"; ?>"><i></i>Level</a></th>
		<th><a href="javascript:void(0)" class="Link_FooTable" data-sort="<?php echo TABLE_MENU."_Status"; ?>"><i></i>Status</a></th>
		<th><a href="javascript:void(0)" class="Link_FooTableActive" data-sort="<?php echo TABLE_MENU."_Ordering"; ?>">
                <i class="glyphicon glyphicon-chevron-up font-10 padding-4"></i>Order</a></th>
	...
	//----------------------------------
		dialogPopWindow = BootstrapDialog.show({
			title: 'เพิ่มเมนูใหม่',
			draggable: true,
			message: $('<div></div>').load('<?php echo SYSTEM_WEBPATH_ROOT; ?>/manage/<?php echo $System_AjaxFileAction; ?>?task=load-add',function(data){
				//runSomeScript();
			}),
	...
	//----------------------------------
		dialogPopWindow = BootstrapDialog.show({
			...
			draggable: true,
			message: $('<div></div>').load('<?php echo SYSTEM_WEBPATH_ROOT; ?>/manage/<?php echo $System_AjaxFileAction; ?>?task=load-edit&myAjaxID='+myID,function(data){
				//runSomeScript();
			}),
			onshown: function(dialogRef){ 
				//runSomeScript();
	...
	//----------------------------------
		dialogPopWindow = BootstrapDialog.show({
			...
			message: $('<div></div>').load('<?php echo SYSTEM_WEBPATH_ROOT; ?>/manage/<?php echo $System_AjaxFileAction; ?>?task=load-confirm&myAjaxID='+myID,function(data){
				//runSomeScript();
			}),
			onshown: function(dialogRef){ 
				//runSomeScript();
