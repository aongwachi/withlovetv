...
if($doaction=="result") {
	...
	//--------------------------------------------------------
	if($search=='') {
		$sql=" SELECT * FROM ".TABLE_CONTENT." WHERE 1  AND ".TABLE_CONTENT."_Status='Draft' AND ".TABLE_CONTENT."_CreateByStaffID='".$SystemSession_Staff_ID."' ";
	} else {
		$sql=" SELECT * FROM ".TABLE_CONTENT." WHERE ".TABLE_CONTENT."_Subject LIKE '%".$search."%'  AND ".TABLE_CONTENT."_Status='Draft' AND ".TABLE_CONTENT."_CreateByStaffID='".$SystemSession_Staff_ID."' ";
	}
	...
	while($Row = mysql_fetch_array($Query)) {
		...
		<td><a href="http://baabinz.com/<?php echo $Row[TABLE_CONTENT."_ID"];?>/" target="_blank">ลิงค์ข่าว</td>		
...
if($doaction=="count") {
	if($search=='') {
		$sql=" SELECT COUNT(*) FROM ".TABLE_CONTENT." WHERE 1 AND ".TABLE_CONTENT."_Status='Draft' AND ".TABLE_CONTENT."_CreateByStaffID='".$SystemSession_Staff_ID."' ";
	}else{
		$sql=" SELECT COUNT(*) FROM ".TABLE_CONTENT." WHERE ".TABLE_CONTENT."_Subject LIKE '%".$search."%'  AND ".TABLE_CONTENT."_Status='Draft' AND ".TABLE_CONTENT."_CreateByStaffID='".$SystemSession_Staff_ID."' ";
	}
	$Query=MYSQL_QUERY($sql,$System_Connection1) OR DIE("Error: ".$sql."<br>\n");
