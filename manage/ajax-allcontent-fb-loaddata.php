...
if($doaction=="result") {
	...
	//--------------------------------------------------------
	if($search=='') {
		$sql=" SELECT * FROM ".TABLE_CONTENT." WHERE 1 AND ".TABLE_CONTENT."_Status='Publish' AND ".TABLE_CONTENT."_Category LIKE '%,26,%' ";
	} else {
		$sql=" SELECT * FROM ".TABLE_CONTENT." WHERE ".TABLE_CONTENT."_Subject LIKE '%".$search."%' AND ".TABLE_CONTENT."_Status='Publish'  AND ".TABLE_CONTENT."_Category LIKE '%,26,%' ";
	}
...
if($doaction=="count") {
	if($search=='') {
		$sql=" SELECT COUNT(*) FROM ".TABLE_CONTENT." WHERE 1 AND ".TABLE_CONTENT."_Status='Publish' AND ".TABLE_CONTENT."_Category LIKE '%,26,%'";
	}else{
		$sql=" SELECT COUNT(*) FROM ".TABLE_CONTENT." WHERE ".TABLE_CONTENT."_Subject LIKE '%".$search."%' AND ".TABLE_CONTENT."_Status='Publish'AND ".TABLE_CONTENT."_Category LIKE '%,26,%' ";
	}
	$Query=MYSQL_QUERY($sql,$System_Connection1) OR DIE("Error: ".$sql."<br>\n");
