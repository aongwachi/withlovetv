...
if(strpos(" ".$Config_DefaultValue,','.$Config_SelectedValue.',')>0) {
    if($Config_SelectedValue==$Config_Array_YesNo_Value[0]) {
        $isChecked=true;
    } else {
        $isChecked=false;
    }
...
$('#input<?php echo $Config_UniqueID; ?>Switch').bootstrapSwitch().on('switchChange.bootstrapSwitch', function(event, state) {
    ...
    OneInput_DoSaveTextOnOffString('<?php echo $Config_UniqueID; ?>','<?php echo $Config_DBx; ?>',$('#input<?php echo $Config_UniqueID; ?>').val(),'<?php echo $Config_SelectedValue; ?>');
});
</script>