...
#########################################################
        ...
        private $type;
        private $extension;
        private $srcDir = '../../upload/cache';
        private $dstDir = '../../upload/avatar';
        private $msg;
		//---------------------------------------------------
        ...
        private function setSrc($src) {
            if (!empty($src)) {
				...
				list($mywidth,$myheight,$type,$myattr)=getimagesize($src);
                if ($type) {
                    $this -> src = $src;
                    $this -> type = $type;
        ...
        private function setFile($file) {
            ...
            if ($errorCode === UPLOAD_ERR_OK) {
				...
				list($mywidth,$myheight,$type,$myattr)=getimagesize($file['tmp_name']);
                if ($type) {
                    $dir = $this -> srcDir;
                    if (!file_exists($dir)) {
                        mkdir($dir, 0777);
