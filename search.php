...
?>
<script>
window.fbAsyncInit = function() { FB.init({ appId : '552453521583555', xfbml : true, version : 'v2.5' }); };
...
<div class="pull-center text-center" style=" max-width:960px; ">
    <table border="0" width="100%"><tr><td class="headbarbg1"></td><td class="text-right webfont headbar headbarbg2" style=" width:180px; "> ค้นหา </td></tr></table>
    ...
    //######################################################################################
    $params = array("%$search%");
    $sql="SELECT COUNT(*) FROM ".TABLE_CONTENT." WHERE ".TABLE_CONTENT."_Subject LIKE ? AND ".TABLE_CONTENT."_Thumb<>'' AND ".TABLE_CONTENT."_Subject<>''  AND ".TABLE_CONTENT."_Text<>''  AND ".TABLE_CONTENT."_Status='Publish' ";
    ...
    $params = array("%$search%");
    $sql="SELECT * FROM ".TABLE_CONTENT." WHERE ".TABLE_CONTENT."_Subject LIKE ? AND ".TABLE_CONTENT."_Thumb<>'' AND ".TABLE_CONTENT."_Subject<>''  AND ".TABLE_CONTENT."_Text<>''  AND ".TABLE_CONTENT."_Status='Publish' ORDER BY ".TABLE_CONTENT."_OnlineDate DESC LIMIT ".$RecordStart.",".$Config_PageSize." ";
    $query=$dbh->prepare($sql);
    ...
    <div class="padding-20 text-center">
        ...
        <?php for($i=1;$i<=$maxpage;$i++) { ?>
            ...
            <?php } else { ?>
                ...
                <a href="/search.php?search=<?php echo $search; ?>&page=<?php echo $i; ?>" class="paginglink"><?php echo $i; ?></a>
                </button>
                <?php } ?>
            <?php } ?>
        <?php } ?>
...
.headbarbg1 { background: url('/templates/bb/img/hbar-bgx.png'); height:40px; }
.headbarbg2 { background: url('/templates/bb/img/hbar-bg1x.png') no-repeat; height:40px; }
...
.fb-share { background: url('/templates/bb/img/bt-fb-shares.png'); width:55px; height:15px; margin-top:4px; }
.fb-share:hover { background: url('/templates/bb/img/bt-fb-share.png'); width:55px; height:15px; margin-top:4px; }

.bt-playarea { height:120px; margin-top:-120px; display:block; }
.bt-play { background: url('/templates/bb/img/play.png'); width:60px; height:60px; opacity: 0.6; display:block; margin-left: auto; margin-right: auto; padding:1px; }
.bt-play:hover { background: url('/templates/bb/img/play.png'); width:60px; height:60px; opacity: 1; display: block;  margin-left: auto; margin-right: auto; padding:1px; }
...
<form name="myPagingForm" id="myPagingForm" method="get" action="/search.php">
